...
<head>
    ...
    <style>
        ...
        .content-row { display:grid; grid-template-columns:500px 1fr; gap:16px; align-items:stretch; }
        .logs-panel { background:var(--surface); border:1px solid var(--border); border-radius:14px; box-shadow:0 2px 12px #0d271514; display:flex; flex-direction:column; height:600px; overflow:hidden; }
        .logs-panel-header { padding:16px 14px 0; flex-shrink:0; }
        .logs-panel-title { font-size:.68rem; letter-spacing:.1em; text-transform:uppercase; color:var(--muted); margin-bottom:12px; display:flex; align-items:center; gap:8px; }
        ...
        .tiny-table th { text-align:left; padding:7px 10px; font-size:.6rem; letter-spacing:.09em; text-transform:uppercase; color:var(--muted); font-weight:500; white-space:nowrap; border-bottom:2px solid var(--border); position:sticky; top:0; background:var(--surface); z-index:1; }
        ...
        .map-panel { background:var(--surface); border:1px solid var(--border); border-radius:14px; box-shadow:0 2px 12px #0d271514; display:flex; flex-direction:column; height:600px; overflow:hidden; }
        .map-panel-header { padding:16px 20px 0; flex-shrink:0; }
        .map-panel-title { font-size:.68rem; letter-spacing:.1em; text-transform:uppercase; color:var(--muted); margin-bottom:10px; display:flex; align-items:center; gap:8px; }
        .map-panel-title::after { content:''; flex:1; height:1px; background:var(--border); }
        .route-strip { padding:8px 20px 10px; font-size:.75rem; display:flex; align-items:center; gap:14px; flex-wrap:wrap; flex-shrink:0; border-bottom:1px solid var(--border); background:var(--surface2); }
        ...
        @media (max-width:900px) { .content-row { grid-template-columns:1fr; } .logs-panel, .map-panel { height:420px; } }
    </style>
</head>
<body>
...
<div class="sidebar">
    ...
    <a href="transportlogsadmin.php" class="nav-btn active">Transport Logs</a>
...
<script>
const T_MIN = 15, T_MAX = 30, H_MIN = 50, H_MAX = 60;
...
let originMarker = null, destMarker = null, routeLayer = null, selectedRow = null;

const greenIcon = L.divIcon({ className:'', html:`<div style="width:13px;height:13px;background:#1a9e3f;border:2px solid #fff;border-radius:50%;box-shadow:0 0 0 3px #22c55e44"></div>`, iconSize:[13,13], iconAnchor:[6,6] });
const redIcon   = L.divIcon({ className:'', html:`<div style="width:13px;height:13px;background:#dc2626;border:2px solid #fff;border-radius:50%;box-shadow:0 0 0 3px #dc262644"></div>`, iconSize:[13,13], iconAnchor:[6,6] });
...
document.querySelectorAll('.delivery-row').forEach(row => {
    row.addEventListener('click', function() {
        ...
        originMarker = L.marker([oLat, oLng], { icon: greenIcon }).addTo(map).bindPopup(`<b>🌾 ${this.dataset.origin}</b><br><small>Origin</small>`);
        destMarker   = L.marker([dLat, dLng], { icon: redIcon   }).addTo(map).bindPopup(`<b>🏪 ${this.dataset.destination}</b><br><small>Destination</small>`);
        ...
        fetch(`https://router.project-osrm.org/route/v1/driving/${oLng},${oLat};${dLng},${dLat}?overview=full&geometries=geojson`)
            ...
            .then(data => {
                ...
                routeLayer = L.geoJSON(data.routes[0].geometry, { style: { color:'#1a9e3f', weight:4, opacity:.9 } }).addTo(map);
                map.fitBounds(routeLayer.getBounds(), { padding:[50,50] });
                map.invalidateSize();
            })
            .catch(() => {
                routeLayer = L.polyline([[oLat,oLng],[dLat,dLng]], { color:'#1a9e3f', weight:3, dashArray:'8 5', opacity:.7 }).addTo(map);
...
function updateStrip(row) {
    ...
    strip.innerHTML = `
        ...
        <div class="ri"><span class="ri-l">Driver</span><span class="ri-v">${row.dataset.driver||'—'}</span></div>
        <div class="ri"><span class="ri-l">Temp</span><span class="ri-v" style="color:${tColor}">${row.dataset.temp||'--'}</span></div>
        <div class="ri"><span class="ri-l">Humidity</span><span class="ri-v" style="color:${hColor}">${row.dataset.hum||'--'}</span></div>
        <div class="ri"><span class="ri-l">Status</span><span class="ri-v">${row.dataset.status}</span></div>
    `;
}
