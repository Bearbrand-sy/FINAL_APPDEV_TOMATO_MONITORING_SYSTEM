...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    if (!$name || !$email || !$contact || !$pass) {
        ...
        if ($check->num_rows > 0) {
            ...
            if ($stmt->execute()) $success = "User \"$name\" added successfully as $role.";
...
<head>
    ...
    <style>
        ...
        .driver-info { background:#fef9e7; border:1px solid #fcd34d; border-radius:8px; padding:10px 14px; font-size:.75rem; color:#92400e; margin-top:12px; display:none; align-items:flex-start; gap:8px; line-height:1.6; }
...
<div class="sidebar">
    ...
    <a href="useradmin.php"          class="nav-btn active">Users</a>
...
<div class="main">
    ...
    <div class="stat-grid">
        <div class="stat-card"><div class="stat-icon">👑</div><div class="stat-label">Admins</div><div class="stat-value purple"><?= $counts['Admin'] ?></div></div>
        <div class="stat-card"><div class="stat-icon">🧑‍💼</div><div class="stat-label">Managers</div><div class="stat-value green"><?= $counts['Manager'] ?></div></div>
        <div class="stat-card"><div class="stat-icon">👤</div><div class="stat-label">Users</div><div class="stat-value blue"><?= $counts['User'] ?></div></div>
        <div class="stat-card"><div class="stat-icon">🚛</div><div class="stat-label">Drivers</div><div class="stat-value amber"><?= $counts['Driver'] ?></div></div>
    </div>
        ...
        <form method="POST">
            ...
            <div class="form-grid">
                <div class="form-field">
                    ...
                    <input type="text" name="name" placeholder="Name Surname" value="<?= htmlspecialchars($_POST['name'] ?? '') ?>">
                </div>
                <div class="form-field">
                    <label>Email</label>
                    <input type="email" name="email" placeholder="name@email.com" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>">
                </div>
                <div class="form-field">
                    <label>Contact Number</label>
                    <input type="text" name="contact" placeholder="09xxxxxxxxx" value="<?= htmlspecialchars($_POST['contact'] ?? '') ?>">
                </div>
                <div class="form-field">
                    <label>Password</label>
                    <div class="pw-wrap">
                        <input type="password" name="password" id="passwordInput" placeholder="Enter password">
                        <button type="button" class="eye-btn" id="eyeBtn" onclick="togglePassword()" title="Show / hide password">👁</button>
                    </div>
                </div>
                <div class="form-field">
                    <label>Role</label>
            ...
            <div class="driver-info" id="driverInfo">
                🚛 <div><b>Driver account note:</b> The <b>Full Name</b> must exactly match the <b>Driver</b> column in the deliveries table (e.g. <code>SANDER PEREJAN</code>).</div>
            </div>
    ...
    <div class="panel">
        <div class="panel-title">Accounts <span style="font-size:.7rem;color:var(--muted);text-transform:none;letter-spacing:0;margin-left:4px;">(<?= count($users) ?> total)</span></div>
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    <?php if (empty($users)): ?>
                    <tr><td colspan="5" style="text-align:center;padding:24px;color:var(--muted);font-size:.78rem;">No users found.</td></tr>
                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td><span class="role-badge <?= strtolower($u['role']) ?>"><?= htmlspecialchars($u['role']) ?></span></td>
                        <td>
...
function onRoleChange(role) {
    document.getElementById("driverInfo").classList.toggle("show", role === "Driver");
}
document.addEventListener("DOMContentLoaded", () => onRoleChange(document.getElementById("roleSelect").value));
</script>
</body>
</html>