...
<head>
    ...
    <style>
        ...
        /* ── content row: narrow table | wide map ── */
        .content-row {
            display: grid;
        ...
        /* map fills remaining space — MUST have explicit height */
        #map {
            flex: 1;
            width: 100%;
...
// Tomato safe ranges: temp 15–30°C, humidity 50–60%
...
function simulateSensors() {
    document.querySelectorAll(".delivery-row").forEach(row => {
        const temp = rnd(T_MIN - 5, T_MAX + 5);   // 10–35
        const hum  = rnd(H_MIN - 6, H_MAX + 6);   // 44–66
...
// Simulate immediately then every 4 seconds
simulateSensors();
...
document.querySelectorAll('.delivery-row').forEach(row => {
    row.addEventListener('click', function() {
        ...
        // Draw OSRM route, fallback to straight dashed line
        ...
        const hum    = this.dataset.hum  || '--';
        const tColor = this.dataset.tOk === '1' ? 'var(--green)' : 'var(--red)';
        const hColor = this.dataset.hOk === '1' ? 'var(--green)' : 'var(--red)';

        strip.className = 'route-strip';
        strip.innerHTML = `
            <div class="ri"><span class="ri-l">ID</span><span class="ri-v">${this.dataset.transport}</span></div>
            <div class="ri"><span class="ri-l">From</span><span class="ri-v">${this.dataset.origin}</span></div>
            <span class="ri-arrow">→</span>
            <div class="ri"><span class="ri-l">To</span><span class="ri-v">${this.dataset.destination}</span></div>
