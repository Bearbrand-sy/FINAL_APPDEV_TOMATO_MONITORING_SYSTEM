...
// ── Auto-create in_transit and delivered tables ───────
$conn->query("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['delivery_id'])) {
    ...
    // Fetch delivery info
    $dRow = $conn->query("SELECT * FROM deliveries WHERE id = $did LIMIT 1")->fetch_assoc();

    if ($dRow) {
        if ($action === 'in_transit') {
            // Update deliveries
            $conn->query("UPDATE deliveries SET status='In Transit' WHERE id=$did");
            // Insert into in_transit (ignore if already exists)
            $stmt = $conn->prepare("
                INSERT IGNORE INTO in_transit
                    (delivery_id, transport_id, product, origin, destination, driver, vehicle_plate, quantity, priority, started_by)
                VALUES (?,?,?,?,?,?,?,?,?,?)
            ");
            ...
            $stmt = $conn->prepare("
                INSERT IGNORE INTO delivered
                    (delivery_id, transport_id, product, origin, destination, driver, vehicle_plate, quantity, priority, avg_temp, avg_hum, total_readings, delivered_by)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
            ");
            // 13 columns: i s s s s s s i s d d i s
            $stmt->bind_param("issssssissdds",
                $did,                    // i  delivery_id
                $dRow['transport_id'],   // s  transport_id
                $dRow['product'],        // s  product
                $dRow['origin'],         // s  origin
                $dRow['destination'],    // s  destination
                $dRow['driver'],         // s  driver
                $dRow['vehicle_plate'],  // s  vehicle_plate
                $dRow['quantity'],       // i  quantity
                $dRow['priority'],       // s  priority
                $avgTemp,                // d  avg_temp
                $avgHum,                 // d  avg_hum
                $totalRead,              // i  total_readings  ← was wrong 'd'
                $byName                  // s  delivered_by
            );
            $stmt->execute();
        }
    }
...
$allDeliveries = $conn->query("
    ...
    ORDER BY
        FIELD(d.status, 'In Transit', 'Pending'),
        d.delivery_date ASC
");
$allRows = [];
while ($row = $allDeliveries->fetch_assoc()) $allRows[] = $row;

// ── Fetch delivered ───────────────────────────────────
$deliveredResult = $conn->query("
    SELECT dv.*, d.notes, d.delivery_date
    FROM delivered dv
    LEFT JOIN deliveries d ON d.id = dv.delivery_id
    ORDER BY dv.delivered_at DESC
");
$deliveredRows = [];
while ($row = $deliveredResult->fetch_assoc()) $deliveredRows[] = $row;

// ── Summary counts ────────────────────────────────────
$totalPending   = count(array_filter($allRows, fn($r) => $r['status'] === 'Pending'));
$totalTransit   = count(array_filter($allRows, fn($r) => $r['status'] === 'In Transit'));
$totalDelivered = count($deliveredRows);
...
<head>
    ...
    <title>GreenGrow · User Dashboard</title>
    ...
    <style>
        ...
        .btn-transit { flex: 1; padding: 11px; border-radius: 9px; border: 2px solid #fcd34d; background: #fef9e7; color: #92400e; font-family: 'DM Mono', monospace; font-size: .78rem; letter-spacing: .06em; text-transform: uppercase; font-weight: 500; cursor: pointer; transition: all .2s; }
        .btn-transit:hover { background: #fef3c7; border-color: #f59e0b; }
        .btn-delivered { flex: 1; padding: 11px; border-radius: 9px; border: none; background: var(--green); color: #fff; font-family: 'DM Mono', monospace; font-size: .78rem; letter-spacing: .06em; text-transform: uppercase; font-weight: 500; cursor: pointer; transition: all .2s; }
        ...
        @media (max-width: 700px) { .stat-grid { grid-template-columns: repeat(2,1fr); } .delivery-grid { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
...
<div class="modal-overlay" id="modalOverlay" onclick="closeModal(event)">
    <div class="modal-box">
        ...
        <div class="modal-section">Driver</div>
        <div class="modal-row"><span>Driver Name</span><b id="m-driver">—</b></div>
        <div class="modal-row"><span>Vehicle Plate</span><b id="m-plate">—</b></div>

        <div class="modal-section">Latest Sensor Readings</div>
...
<div class="sidebar">
    ...
    <a href="user.php"          class="nav-btn active">Dashboard</a>
    <a href="transportuser.php" class="nav-btn">Transport Logs</a>
...
<div class="main">
    <div class="page-header">
        <h1>Dashboard</h1>
        <p>Delivery monitoring &amp; status management</p>
    </div>
    ...
    <div class="stat-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value blue"><?= $totalTransit ?></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon">✅</div>
            <div class="stat-label">Delivered</div>
            <div class="stat-value green"><?= $totalDelivered ?></div>
        </div>
    </div>
    ...
    <div class="panel">
        <div class="panel-title">
            Active Deliveries
            <span style="font-size:.7rem;color:var(--muted);text-transform:none;letter-spacing:0;margin-left:4px;"><?= $totalAll ?> records · click a card to manage</span>
        </div>

        <?php if (empty($allRows)): ?>
        <div class="empty-state">
            <div class="empty-icon">📦</div>
            <p>No active deliveries. The manager will create them.</p>
        </div>
        <?php else: ?>
        <div class="delivery-grid">
            <?php foreach ($allRows as $d):
                ...
                $statusBadge = $d['status'] === 'In Transit' ? 'b-transit' : 'b-pending';
            ?>
            <div class="delivery-card <?= $cardCls ?>"
                ...
                <div class="dc-header">
                    ...
                    <span class="badge <?= $statusBadge ?>"><?= htmlspecialchars($d['status']) ?></span>
                </div>

                <div class="dc-route">
                    🍅 <b><?= htmlspecialchars($d['product']) ?></b> · <?= (int)$d['quantity'] ?> crates<br>
                    📍 <?= htmlspecialchars($d['origin']) ?> → <?= htmlspecialchars($d['destination']) ?><br>
                    👤 <?= htmlspecialchars($d['driver'] ?? '—') ?> · 📅 <?= htmlspecialchars($d['delivery_date']) ?>
                </div>
                ...
                <div class="dc-footer">
                    ...
                    <span><?= $d['last_reading'] ? '🕐 '.date('M d, H:i', strtotime($d['last_reading'])) : 'No readings' ?></span>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
    ...
    <div class="panel">
        <div class="panel-title">
            Delivered
            <span style="font-size:.7rem;color:var(--muted);text-transform:none;letter-spacing:0;margin-left:4px;">(<?= $totalDelivered ?> records)</span>
        </div>
        <?php if (empty($deliveredRows)): ?>
        <div class="empty-state">
            <div class="empty-icon">✅</div>
        ...
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    <?php foreach ($deliveredRows as $d):
                    ...
                    <tr>
                        ...
                        <td style="font-size:.72rem;"><?= htmlspecialchars($d['driver'] ?? '—') ?></td>
                        ...
                        <td style="font-size:.72rem;"><?= date('M d, Y H:i', strtotime($d['delivered_at'])) ?></td>
                        <td style="font-size:.72rem;"><?= htmlspecialchars($d['delivered_by'] ?? '—') ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
function openModal(d) {
    ...
    if (d.status === "In Transit") {
        // Already in transit: disable that button
        btnT.classList.add("btn-disabled");
        btnD.classList.remove("btn-disabled");
    } else {
        // Pending: both available
        btnT.classList.remove("btn-disabled");
        btnD.classList.remove("btn-disabled");
    }
